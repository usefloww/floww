...
export class Gitlab extends BaseProvider {
    ...
    private getBaseUrl(): string {
        return this.getConfig<string>('baseUrl', 'https://gitlab.com') || 'https://gitlab.com';
    }
                ...
                setup: async (ctx) => {
                    ...
                    try {
                        ...
                        if (args.projectId) {
                            ...
                            console.log(`✅ GitLab webhook registered for project ${args.projectId}`);
                            ...
                            console.log(`✅ GitLab webhook registered for group ${args.groupId}`);
                        ...
                        console.error('❌ Failed to register GitLab webhook:', error.message);
                        throw error;
                    }
                },
                teardown: async (ctx) => {
                    ...
                    try {
                        if (projectId) {
                            ...
                            console.log(`🗑️  GitLab webhook ${webhookId} deleted from project ${projectId}`);
                        } else if (groupId) {
                            await api.deleteGroupWebhook(groupId, webhookId);
                            console.log(`🗑️  GitLab webhook ${webhookId} deleted from group ${groupId}`);
                        }
                    } catch (error: any) {
